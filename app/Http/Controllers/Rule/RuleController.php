...
class RuleController extends Controller
{
    ...
    public function list(request $request,response $response){
        ...
        $prevMonthCount = Rule::where('created_at','<', Carbon::now()->subMonthsNoOverflow()->endOfMonth()->toDateString())->count();
        $totalCount = Rule::All()->count();
        $percentageRule = (($totalCount-$prevMonthCount)/$totalCount)*100;
        $data = $RuleResource;
        $config = [
                    'page'   => [
                    ...
                    'stat'=>[
                        'Total Instructor'   => [
                            'name'=> 'Total Package Rule',
                            'width' => 'col-md-12',
                            'icon'=> 'ri-user-3-line font-size-24',
                            'module'=> 'rule',
                            'count-value'=> $totalCount,
                            'percentage-value'=>  H1BHelper::isHasDecimal($percentageRule).'%',                            
