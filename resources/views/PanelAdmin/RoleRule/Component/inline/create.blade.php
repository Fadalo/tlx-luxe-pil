...
//exit();
?>
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasRoleRule" aria-labelledby="offcanvasRoleRuleLabel">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasRoleRuleLabel">Create Role Rule</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body">
        <hr>
        <div>
            Fill form below to create new Role
        </div>
        <hr>
        <form id="formRoleRule" method="POST" autocomplete="off" action="{{ route('admin.RoleRule.create') }}" >
            @csrf
            <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const saveButton = document.querySelector('#formRoleRule button[name="btnSave"]');
    const saveClear  = document.querySelector('#formRoleRule button[name="btnClear"]');
    // Optional: Add an event listener to the button
    saveButton.addEventListener('click', function(event) {
         // Prevent form submission for demonstration
        ajaxable('#formRoleRule').onStart(function(params) {
        ...
        .onResponse(function(res, params) {
            ...
            $('#offcanvasRoleRule .btn-close').click();
            Swal.fire({
                title: 'Success',
                text: result.message,
                icon: 'success',
    ...
    saveClear.addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('formRoleRule').reset();
    });
