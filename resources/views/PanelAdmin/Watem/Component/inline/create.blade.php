...
//exit();
?>
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasWatem" aria-labelledby="offcanvasTemplateLabel">
    ...
    <div class="offcanvas-body">
        ...
        <hr>
        <form id="formWatem" method="POST" autocomplete="off" action="{{ route('admin.watem.create') }}" >
            @csrf
            <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const saveButton = document.querySelector('#formWatem button[name="btnSave"]');
    const saveClear  = document.querySelector('#formWatem button[name="btnClear"]');
    // Optional: Add an event listener to the button
    saveButton.addEventListener('click', function(event) {
         // Prevent form submission for demonstration
        ajaxable('#formWatem').onStart(function(params) {
    ...
    saveClear.addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('formWatem').reset();
    });
