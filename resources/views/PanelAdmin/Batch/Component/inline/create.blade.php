...
//exit();
?>
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasBatch" aria-labelledby="offcanvasPackageLabel">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasBatchLabel">Create Batch</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body">
        <hr>
        <div>
            Fill form below to create new Batch
        </div>
        <hr>
        <form id="formBatch" method="POST" autocomplete="off" action="{{ route('admin.batch.create') }}" >
            @csrf
            <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const saveButton = document.querySelector('#formBatch button[name="btnSave"]');
    const saveClear  = document.querySelector('#formBatch button[name="btnClear"]');
    // Optional: Add an event listener to the button
    saveButton.addEventListener('click', function(event) {
         // Prevent form submission for demonstration
        ajaxable('#formBatch').onStart(function(params) {
        ...
        .onResponse(function(res, params) {
            ...
            $('#offcanvasBatch .btn-close').click();
            Swal.fire({
                title: 'Success',
                text: result.message,
                icon: 'success',
