@php
    //$submoduleMyPackage = $config['relation'][0]['submodule'];
    //$data = $config['relation'][0]['submodule']['tabs']['data'];
    //print_r('<pre>');
    //print_r($submoduleMyPackage);
    //exit();
@endphp



<?php
// <livewire:ScheduleInput /><livewire:DynamicGrid :config="$config" /><!-- <livewire:Component.GridWithCrud.InlineGridCreate /> -->//@include($submoduleMyPackage['tabs']['render'],$data)
?>


<livewire:InstructorContractPackage :config="$config" :instructor_id="$config['id']" />
