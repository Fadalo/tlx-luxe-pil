...
//exit();
?>
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasMenuAdmin" aria-labelledby="offcanvasMenuAdminLabel">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasMenuAdminLabel">Create Menu Admin</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body">
        <hr>
        <div>
            Fill form below to create new Menu Admin
        </div>
        <hr>
        <form id="formMenuAdmin" method="POST" autocomplete="off" action="{{ route('admin.MenuAdmin.create') }}" >
            @csrf
            <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const saveButton = document.querySelector('#formMenuAdmin button[name="btnSave"]');
    const saveClear  = document.querySelector('#formMenuAdmin button[name="btnClear"]');
    // Optional: Add an event listener to the button
    saveButton.addEventListener('click', function(event) {
         // Prevent form submission for demonstration
        ajaxable('#formMenuAdmin').onStart(function(params) {
        ...
        .onResponse(function(res, params) {
            ...
            $('#offcanvasMenuAdmin .btn-close').click();
            Swal.fire({
                title: 'Success',
                text: result.message,
                icon: 'success',
    ...
    saveClear.addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('formMenuAdmin').reset();
    });
